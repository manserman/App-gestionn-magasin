...
public class Commandes extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    DrawerLayout drawerlayout;
    ActionBarDrawerToggle actionBarDrawerToggle;
    FloatingActionButton ajouter;
    NavigationView nav_view;
    ListView lv;
    Commandes_adapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linear=findViewById(R.id.layout_commande);
        ...
        if(bd.commande()!=null)
        {adapter=new Commandes_adapter(this,R.layout.list_items,bd.commande());
    ...
    {super.onResume();
        ...
        admins=bd.commande();
        if(admins!=null)
        {adapter=new Commandes_adapter(this,R.layout.list_items,admins);
        lv.setAdapter(adapter);}
        ajouter.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View t){
                ...
                cursor.close();
                  ...
                  if(bd.clients()!=null)
                  {if(bd.produits()!=null)
                  {Intent intent=new Intent(getApplicationContext(),Ajout_commande.class);
                intent.putExtras(getIntent().getExtras());
                startActivity(intent);}
                  else
                  {
                      AlertDialog.Builder dlgAlert  = new AlertDialog.Builder(Commandes.this);;
                      dlgAlert.setMessage("Votre stock est vide. Veuillez ajouter au moins un produit svp").setTitle("Aucun produit dans la base").show();

                  }}
            else
                  {
                      AlertDialog.Builder dlgAlert  = new AlertDialog.Builder(Commandes.this);
                      dlgAlert.setMessage("Votre liste de clients est vide. Veuillez ajouter au moins un client svp").setTitle("Aucun client dans la base").show();
                  }
            }
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                infos_commande dialog=new infos_commande();
    ...
    {return true;}
        ...
        { case R.id.recherche:
            ...
            case R.id.tri_id_croissant:
                ...
                String sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY ID ASC;";
            ...
            case R.id.tri_id_decroissant:
                item.setChecked(true);
                prods=new ArrayList<>();
                sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY ID DESC;";
            ...
            case R.id.tri_prix_croissant:
                item.setChecked(true);
                prods=new ArrayList<>();
                sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY PRIXTOTAL ASC;";
                cursor=new  MySQLiteOpenHelper(getBaseContext(),null).getReadableDatabase().rawQuery(sql,null);
                n=cursor.getCount();
                if(n<=0)
                    prods=null;
            ...
            case R.id.tri_prix_decroissant:
                item.setChecked(true);
                prods=new ArrayList<>();
                sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY PRIXTOTAL DESC;";
                cursor=new  MySQLiteOpenHelper(getBaseContext(),null).getReadableDatabase().rawQuery(sql,null);
                n=cursor.getCount();
                if(n<=0)
                    prods=null;
            ...
            case R.id.tri_date_croissant:
                item.setChecked(true);
                prods=new ArrayList<>();
                sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY DATE ASC;";
                cursor=new  MySQLiteOpenHelper(getBaseContext(),null).getReadableDatabase().rawQuery(sql,null);
                n=cursor.getCount();
                if(n<=0)
                    prods=null;
            ...
            case R.id.tri_date_decroissant:
                item.setChecked(true);
                prods=new ArrayList<>();
                sql= "select ID,IDCLIENT,PRIXTOTAL,PAYEE,IDADMIN,DATE from COMMANDE ORDER BY DATE DESC;";
                cursor=new  MySQLiteOpenHelper(getBaseContext(),null).getReadableDatabase().rawQuery(sql,null);
                n=cursor.getCount();
                if(n<=0)
                    prods=null;
