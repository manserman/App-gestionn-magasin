...
public class Ajout_client extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener{
    Button effectuer;
    Button annuler;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        annuler=findViewById(R.id.annuler_client);
        effectuer=findViewById(R.id.effectuer_);
        drawerlayout=(DrawerLayout)findViewById(R.id.drawerlayout_ajout_client);
        ...
        Toolbar toolbar = findViewById(R.id.toolbar_ajoutclient);
        toolbar.setTitleTextColor(getResources().getColor(R.color.white));
        if(type==1)
        toolbar.setTitle("Modifier les infos d'un client");
        else  toolbar.setTitle("Ajouter un client");
        setSupportActionBar(toolbar);
        actionBarDrawerToggle = new ActionBarDrawerToggle(this, drawerlayout, toolbar, R.string.drawer_open, R.string.drawer_close);
        drawerlayout.addDrawerListener(actionBarDrawerToggle);
        actionBarDrawerToggle.getDrawerArrowDrawable().setColor(getResources().getColor(R.color.white));
        actionBarDrawerToggle.syncState();
        Nom=findViewById(R.id.nom_ajout_client);
        Prenom=findViewById(R.id.prenom_ajout_client);
        telephone=findViewById(R.id.tel_ajout_client);
        Mail=findViewById(R.id.mail_ajout_client);
        adresse=findViewById(R.id.adresses_ajout_client);
        if(type==1)
        {id0=getIntent().getExtras().getInt("id");
            MySQLiteOpenHelper data=new MySQLiteOpenHelper(this,null);
        SQLiteDatabase db=data.getReadableDatabase();
        Cursor cursor=db.rawQuery("SELECT NOM,PRENOM,TELEPHONE,MAIL,ADRESSE FROM CLIENT WHERE ID="+id0+";",null);
        cursor.moveToFirst();
    ...
    {super.onResume();
        ...
        effectuer.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View t){
                if(!Nom.getText().toString().isEmpty() && !Prenom.getText().toString().isEmpty() && !Mail.getText().toString().isEmpty() && !adresse.getText().toString().isEmpty() && !telephone.getText().toString().isEmpty() )
               ...
               if(type==0) {
                  ...
                  db.execSQL("INSERT INTO CLIENT VALUES((SELECT MAX(ID) FROM CLIENT)+1,'"+Nom.getText().toString()+"','"+Prenom.getText().toString()+"','"+telephone.getText().toString()+"','"+Mail.getText().toString()+"','"+adresse.getText().toString()+"');");
               }
               else
               ...
               db.update("CLIENT",cv,"ID=?",new String[] {String.valueOf(id0)});
               db.close();
