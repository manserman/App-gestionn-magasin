...
public class Ajout_commande extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle("Ajouter une Commande");
    ...
    {super.onResume();
        ...
        effect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                    ...
                    if (!qte.getText().toString().isEmpty()) {
                        if (Integer.valueOf(qte.getText().toString()) <= Integer.valueOf(produits.get(pos)[7])) {
                            String[] infos = new String[7];
                            infos[0] = j.toString();
                            j++;
                            infos[1] = produits.get(pos)[1];
                            infos[2] = qte.getText().toString();
                            infos[3] = produits.get(pos)[2];
                            ...
                            Toast.makeText(getApplicationContext(), "Désolé nous ne disposons pas de cette quantité pour ce produit", Toast.LENGTH_LONG).show();
                    } else
        ...
        effetuer.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Cursor cursor=database.rawQuery("SELECT * FROM COMMANDE;",null);
                if(cursor.getCount()==0)
                {
                    ...
                    curs.moveToFirst();
                str="INSERT INTO COMMANDE VALUES ("+0+","+clienteles.get(pos1)[0]+","+ curs.getString(0)+",DATE(),"+total.getText().toString()+",'false');";
                database.execSQL(str);
                }
                else
                { Cursor curs=database.rawQuery("SELECT ID FROM ADMINISTRATEURS WHERE USERNAME=?",new String[]{getIntent().getExtras().getString("username")});
                    curs.moveToFirst();
                    Cursor curs1=database.rawQuery("SELECT MAX (ID) FROM COMMANDE;",null);
                    curs1.moveToFirst();
                    int id=Integer.valueOf(curs1.getString(0))+1;
                    str="INSERT INTO COMMANDE VALUES ("+id+","+clienteles.get(pos1)[0]+","+curs.getString(0)+",(DATE()),"+total.getText().toString()+",'false');";
                database.execSQL(str);
                ...
                for (String[] n : prods) {
                    ...
                    Cursor cur=database.rawQuery("SELECT QUANTITE FROM PRODUCTS WHERE ID=? AND LOT=?",new String[]{n[5],n[6]});
                    cur.moveToFirst();
                    Integer qte=Integer.valueOf(cur.getString(0))-Integer.valueOf(n[2]);
                    cv.put("QUANTITE",qte);
                    cur.close();
                    Cursor curs1=database.rawQuery("SELECT MAX (ID) FROM COMMANDE;",null);
                    curs1.moveToFirst();
                    database.update("PRODUCTS",cv,"ID=? AND LOT=?",new String[]{n[5],n[6]});
                    String ex="INSERT INTO LIGNECOMMANDE VALUES ("+curs1.getString(0)+","+n[5]+","+n[6]+","+n[2]+","+n[3]+");";
                    database.execSQL(ex);
                    curs1.close();
    ...
    else { AlertDialog.Builder dlgAlert  = new AlertDialog.Builder(getApplicationContext());
