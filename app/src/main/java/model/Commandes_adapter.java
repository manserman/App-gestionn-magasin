...
public class Commandes_adapter extends ArrayAdapter<String[]> implements Filterable {
    Context context;
    int m_ressource;
    ArrayList<String[]> prod;
    ArrayList<String[]> produits1;
    public  Commandes_adapter(Context context,int ressource, ArrayList<String[]> produits)
    {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        Cursor curs=database.rawQuery("SELECT NOM,PRENOM FROM CLIENT WHERE ID=?",new String[]{produit[1]});
        curs.moveToFirst();
        desc.setText("Client: "+curs.getString(0)+" "+curs.getString(1));
        curs.close();
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (int i = 0; i < produits1.size(); i++) {
                    ...
                    Cursor curs=new MySQLiteOpenHelper(getContext(),null).getReadableDatabase().rawQuery("SELECT NOM,PRENOM FROM CLIENT WHERE ID=?",new String[]{item[1]});
                    curs.moveToFirst();
